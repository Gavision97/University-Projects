public class MinimumSegmentTreeByArray extends SegmentTreeByArray{
    ...
    /**
     * The method builds the minimum segment tree by array using recursive helping function
    ...
    protected int query(int node, int start, int end, int left, int right) {
        if(left <= start && right >=  end) return this.tree[node];
            ...
            return Math.min(leftNode, rightNode); // return the sum of the two nodes
        }
    }
}
